function merge(folderId, pdfList, outputName) {
  //フォルダオブジェクト
  var folder = DriveApp.getFolderById(folderId);

  // PDFファイルをマージ
  return mergePdfs(folder, outputName, pdfList);
}

function mergeTestCall() {
  const FOLDER_ID = '1hNxVSvHjpkXbfgxsMtUP7UZwUKV51Jnu'; //フォルダ ID
  ...
  return mergePdfs(folder, '合体したPDFファイル.pdf', pdfList);
}
 ...
 * Edited to work in V8  (Apr 16, 2020)
 * References:
...
function mergePdfs(directory, name, pdf_list) {
  ...
  for (var argumentIndex in pdf_list) {
    ...
    do {
      ...
      if (objectByteOffsets.length < 1) {
        rootAddress = /\d+\s+\d+\s+R/
          .exec(/\/Root\s*\d+\s+\d+\s+R/.exec(trailerDictionary)[0])[0]
          .replace('R', 'obj');
      }
      ...
      if (/\s*\/Prev/.test(trailerDictionary)) {
        xrefByteOffset = +/\s*\d+\s/.exec(
          /\s*\/Prev\s*\d+\s/.exec(trailerDictionary)[0]
        )[0];
      }
    } while (/\s*\/Prev/.test(trailerDictionary));
    var rootObject = getObject(rootAddress, objectByteOffsets, bytes);
    var pagesAddress = /\d+\s+\d+\s+R/
      .exec(/\/Pages\s*\d+\s+\d+\s+R/.exec(rootObject)[0])[0]
      .replace('R', 'obj');
    ...
    for (var objectIndex = 0; objectIndex < objects.length; objectIndex++) {
      ...
      var objectAddress = objects[objectIndex][1]
        .match(/\d+\s+\d+\s+obj/)[0]
        .split(/\s+/);
      objects[objectIndex].splice(1, 0, objectAddress);
      if (/\/Type\s*\/Page[^s]/.test(objects[objectIndex][2])) {
        objects[objectIndex][2] = objects[objectIndex][2].replace(
          /\/Parent\s*\d+\s+\d+\s+R/.exec(objects[objectIndex][2])[0],
          '/Parent 2 0 R'
        );
        pageAddresses.push(newObjectAddress.join(' ').replace('obj', 'R'));
      }
      var addressRegExp = new RegExp(
        objectAddress[0] + '\\s+' + objectAddress[1] + '\\s+' + 'obj'
      );
      objects[objectIndex][2] = objects[objectIndex][2].replace(
        addressRegExp.exec(objects[objectIndex][2])[0],
        newObjectAddress.join(' ')
      );
      newObjects.push(objects[objectIndex]);
    }
    for (
      var referencingObjectIndex = newObjectsInsertionIndex;
      referencingObjectIndex < newObjects.length;
      referencingObjectIndex++
    ) {
      var references =
        newObjects[referencingObjectIndex][2].match(/\d+\s+\d+\s+R/g);
      if (references != null) {
        ...
        for (
          ...
          referenceIndices[referenceIndex].push(
            string.slice(currentIndex).indexOf(references[referenceIndex]) +
              currentIndex
          );
          referenceIndices[referenceIndex].push(
            references[referenceIndex].length
          );
          currentIndex += string
            .slice(currentIndex)
            .indexOf(references[referenceIndex]);
        }
        for (
          var referenceIndex = 0;
          referenceIndex < references.length;
          ...
          var objectAddress = references[referenceIndex]
            .replace('R', 'obj')
            .split(/\s+/);
          for (
            var objectIndex = newObjectsInsertionIndex;
            objectIndex < newObjects.length;
            objectIndex++
          ) {
            if (arrayEquals(objectAddress, newObjects[objectIndex][1])) {
              var length = string.length;
              newObjects[referencingObjectIndex][2] =
                string.slice(0, referenceIndices[referenceIndex][0]) +
                newObjects[objectIndex][0].join(' ').replace('obj', 'R') +
                string.slice(
                  referenceIndices[referenceIndex][0] +
                    referenceIndices[referenceIndex][1]
                );
              string = newObjects[referencingObjectIndex][2];
              var newLength = string.length;
              if (!(length == newLength)) {
                for (
                  var subsequentReferenceIndex = referenceIndex + 1;
                  subsequentReferenceIndex < references.length;
                  subsequentReferenceIndex++
                ) {
                  referenceIndices[subsequentReferenceIndex][0] +=
                    newLength - length;
                }
              }
              break;
  ...
  newObjects.splice(
    ...
    '3 0 obj\r\n<</Title (' +
      name +
      ') /Producer (PdfManipulation.mergePdfs\\(\\), a Google Apps Script project by Jarom Luker \\(pricebook@hbboys.com\\)) /CreationDate (D' +
  ...
  while (newObjects.length > 0) {
    ...
    if (Array.isArray(object)) {
      ...
      for (
        var objectIndex = streamIndex;
        objectIndex < object[0].length;
        objectIndex++
      ) {
        bytes.push(object[0].charCodeAt(objectIndex));
      }
  ...
  var xrefHeader = 'xref\r\n';
  for (
    var xrefHeaderIndex = 0;
    xrefHeaderIndex < xrefHeader.length;
    xrefHeaderIndex++
  ) {
  ...
  for (
    var xrefSectionHeaderIndex = 0;
    xrefSectionHeaderIndex < xrefSectionHeader.length;
    xrefSectionHeaderIndex++
  ) {
    bytes.push(xrefSectionHeader.charCodeAt(xrefSectionHeaderIndex));
  }
  for (
    var byteOffsetIndex = 0;
    byteOffsetIndex < byteOffsets.length;
    byteOffsetIndex++
  ) {
    for (
      var byteOffsetStringIndex = 0;
      byteOffsetStringIndex < 10;
      byteOffsetStringIndex++
    ) {
      bytes.push(
        Utilities.formatString(
          '%010d',
          byteOffsets[byteOffsetIndex]
        ).charCodeAt(byteOffsetStringIndex)
      );
    }
    bytes.push(' '.charCodeAt(0));
    if (byteOffsetIndex == 0) {
      for (
        var generationStringIndex = 0;
        generationStringIndex < 5;
        generationStringIndex++
      ) {
  ...
  for (
    var trailerHeaderIndex = 0;
    trailerHeaderIndex < 9;
    trailerHeaderIndex++
  ) {
    bytes.push('trailer\r\n'.charCodeAt(trailerHeaderIndex));
  }
  var idBytes = Utilities.computeDigest(
  ...
  return directory.createFile(
    Utilities.newBlob(bytes, 'application/pdf', name)
  );
  function getObject(objectAddress, objectByteOffsets, bytes) {
    ...
    while (object[0].indexOf('endobj') <= -1) {
      if (/stream\s*\n/.test(object[0])) {
        var streamLength;
        var lengthFinder = object[0].slice(
          object[0].indexOf(/\/Length/.exec(object[0])[0])
        );
        if (/\/Length\s*\d+\s+\d+\s+R/.test(lengthFinder)) {
          var lengthObjectAddress = /\d+\s+\d+\s+R/
            .exec(/\/Length\s*\d+\s+\d+\s+R/.exec(lengthFinder)[0])[0]
            .split(/\s+/);
          lengthObjectAddress[2] = 'obj';
          for (var addressIndex = 0; addressIndex < 2; addressIndex++) {
            lengthObjectAddress[addressIndex] =
              +lengthObjectAddress[addressIndex];
          }
          var lengthObject = '';
          ...
          while (lengthObject.indexOf('endobj') <= -1) {
            lengthObject =
              lengthObject + String.fromCharCode(bytes[lengthByteIndex]);
            lengthByteIndex++;
          }
          streamLength = +lengthObject
            .match(/obj\s*\n\s*\d+\s*\n\s*endobj/)[0]
            .match(/\d+/)[0];
        } else {
    ...
    if (references != null) {
      while (references.length > 0) {
        if (
          /\/Parent/.test(
            objectString.slice(
              objectString.indexOf(references[0]) - 8,
              objectString.indexOf(references[0])
            )
          )
        ) {
          references.shift();
        ...
        var dependency = getObject(
          references.shift().replace('R', 'obj'),
          objectByteOffsets,
          bytes
        );
        ...
        for (var key in dependencies) {
          ...
          dependencyExists = arrayEquals(dependency, entry)
            ? true
            : dependencyExists;
        }
        if (!dependencyExists) {
        ...
        if (Array.isArray(dependency)) {
          dependencies = dependencies.concat(
            getDependencies(dependency[0], objectByteOffsets, bytes)
          );
        } else {
          dependencies = dependencies.concat(
            getDependencies(dependency, objectByteOffsets, bytes)
          );
        }
      }
    }
...
function splitPdf(pdf, pageRange1, opt_pageRange2) {
  ...
  var pagesAddress = /\d+\s+\d+\s+R/
    ...
    .replace('R', 'obj');
  var pagesObject = getObject(pagesAddress, objectByteOffsets, bytes);
  var pagesString = pagesObject.slice(
    pagesObject.indexOf(/\/Kids\s*\[/.exec(pagesObject)[0])
  );
  pagesString = pagesString.slice(
    pagesString.indexOf('[') + 1,
    pagesString.indexOf(']')
  );
  var pageAddresses = [];
  ...
  for (
    ...
    if (range.indexOf('-') > -1) {
      ...
      if (end == 'end') {
        for (
          var pageNumber = begin;
          pageNumber <= pageAddresses.length;
          pageNumber++
        ) {
          ...
          objects = objects.concat(
            getDependencies(page, objectByteOffsets, bytes)
          );
        }
      } else {
        end = +end;
  ...
  newObjects.splice(
    ...
    '3 0 obj\r\n<</Title (' +
      name +
      ') /Producer (PdfManipulation.splitPdf\\(\\), a Google Apps Script project by Jarom Luker \\(pricebook@hbboys.com\\)) /CreationDate (D' +
      Utilities.formatDate(
        new Date(),
        CalendarApp.getDefaultCalendar().getTimeZone(),
        'yyyyMMddHHmmssZ'
  ...
  var directory = pdf.getParents().next();
  return directory.createFile(
    Utilities.newBlob(bytes, 'application/pdf', name)
  );
  function getObject(objectAddress, objectByteOffsets, bytes) {
